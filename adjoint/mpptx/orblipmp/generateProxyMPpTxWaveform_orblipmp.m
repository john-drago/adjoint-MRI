function [ waveform ] = generateProxyMPpTxWaveform_orblipmp( p, opt )
...
if numZCoils > 0
    ...
    Shim( :, shim_Blip_slew_i_idxs ) = ( shim_Blip / opt.BlipShimSlewTime ) *...
        ( tvec( shim_Blip_slew_i_idxs ) - opt.tStBlip );
    Shim( :, shim_Blip_slew_f_idxs ) = -( shim_Blip / opt.BlipShimSlewTime ) *...
        ( tvec( shim_Blip_slew_f_idxs ) - opt.tEndBlip );

    Shim( :, shim_Blip_const_idxs ) = repmat( shim_Blip, [ 1 length( shim_Blip_const_idxs ) ] );
