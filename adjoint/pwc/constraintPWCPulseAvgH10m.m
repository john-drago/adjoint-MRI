function [ c, gradc ] = constraintPWCPulseAvgH10m( pSc, opt )
...
if ~ismatrix( VOPs_H10m )
    avgH10m = sqrt( real( squeeze( sum( VOPs_H10m .* b_pwr, [ 1 2 ] ) ) ) );
else
    avgH10m = sqrt( real( VOPs_H10m .* b_pwr ) );
end

c_unsc = ( (dutyCycle) / numTimePoints ) * ( avgH10m ) - avgH10mconstr;
c = c_unsc / avgH10mconstr;

if nargout > 1
    
    % Initialize gradient vector
    gradc_unsc = zeros( opt.numVars, numVOPs_H10m );
    ...
    if all( avgH10m == 0, "all" )
        avgH10m = ones( numVOPs_H10m, 1 );
    end
    if ~ismatrix( VOPs_H10m )
        gradf_zbar_rshp = ( ( (dutyCycle) / numTimePoints ) * reshape( 1./avgH10m, [ 1, 1, numVOPs_H10m ] ) ) .*...
            tensorprod( bcomp_rshp, conj( VOPs_H10m ), 2, 1 );
    else
        gradf_zbar_rshp = ( ( (dutyCycle) / ( numTimePoints * avgH10m ) ) )...
            * ( bcomp_rshp * conj( VOPs_H10m ) );
    end
    ...
    breal_idx_rshp_rep = reshape( repmat( breal_idx_rshp, [ 1, 1, numVOPs_H10m ] ),...
        [ numTimePoints*numXYCoils, numVOPs_H10m ] );
    bimag_idx_rshp_rep = reshape( repmat( bimag_idx_rshp, [ 1, 1, numVOPs_H10m ] ),...
        [ numTimePoints*numXYCoils, numVOPs_H10m ] );
    vop_idx_rshp_rep = reshape( repmat( reshape( 1:numVOPs_H10m, [ 1, 1, numVOPs_H10m  ] ),...
        [ numTimePoints, numXYCoils ] ), [ numTimePoints*numXYCoils, numVOPs_H10m ] );
    ...
    gradc = gradc_unsc / avgH10mconstr;
end
end