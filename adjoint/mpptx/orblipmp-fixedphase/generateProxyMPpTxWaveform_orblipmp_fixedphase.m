...
cosdwxy = cos( dwxy * ( tvec( MPSP_idxs ) - opt.tStMPSP) );
sindwxy = sin( dwxy * ( tvec( MPSP_idxs ) - opt.tStMPSP) );
...
Grad( :, G_MPSP_slew_i_idxs ) = ( ( G_mag_MPSP / opt.MPSPGradSlewTime ) .*...
    cos( wz * opt.MPSPGradSlewTime +  G_ph_MPSP ) ) *...
    ( tvec( G_MPSP_slew_i_idxs ) - opt.tStMPSP );
Grad( :, G_MPSP_slew_f_idxs ) = -( ( G_mag_MPSP / opt.MPSPGradSlewTime ) .*...
    cos( wz * ( opt.tEndMPSP - opt.tStMPSP - opt.MPSPGradSlewTime ) +  G_ph_MPSP ) ) *...
    ( tvec( G_MPSP_slew_f_idxs ) - opt.tEndMPSP );
Grad( :, G_MPSP_wave_idxs ) = G_mag_MPSP .* ( cos( wz * ( tvec( G_MPSP_wave_idxs ) - opt.tStMPSP ) + G_ph_MPSP ) );
...
if numZCoils > 0
    ...
    Shim( :, shim_MPSP_slew_i_idx ) = ( ( shim_mag_MPSP / opt.MPSPShimSlewTime ) .*...
        cos( wz * opt.MPSPShimSlewTime +  shim_ph_MPSP ) )*...
        ( tvec( shim_MPSP_slew_i_idx ) - opt.tStMPSP );
    Shim( :, shim_MPSP_slew_f_idx ) = -( ( shim_mag_MPSP / opt.MPSPShimSlewTime ) .*...
        cos( wz * ( opt.tEndMPSP - opt.tStMPSP - opt.MPSPShimSlewTime ) +  shim_ph_MPSP ) )*...
        ( tvec( shim_MPSP_slew_f_idx ) - opt.tEndMPSP );
    Shim( :, shim_MPSP_wave_idx ) = shim_mag_MPSP .* ( cos( wz * ( tvec( shim_MPSP_wave_idx ) - opt.tStMPSP ) + shim_ph_MPSP ) );
