...
note = [...
    ...
    "Piecewise constant 90 degree flip angle with no bandwidth constraint" ];
...
savePathTop = fullfile( tld, 'data', 'opt',...
    ...
    '2025-04-04-tailored-spokes-init-90-pwc',...
    strcat( 'opt', timestr ) );

prevOptDirPath = fullfile( tld, "data", "opt",...
    "2025-04-03-universal-selective",...
    "2025-04-03-universal-spokes-init-90-pwc",...
    "XXXXXXXX" );
...
st.spacingTrackDecisionVariables = 1;
...
for ll = 1:length( sliceLocFolderPaths )
    ...
    for dd = 1:length( optTypeFolderPaths )
        ...
        for tt = 1:numTest
            ...
            maxOptTime = 20 * ( 60 );
            maxOptIterationsSQP = 2e3;
            maxOptIterationsIP = 2e3;
            ...
            if matches( solverName, 'active-set' )
                ...
                oc.fminopt.RelLineSrchBnd = 5.0e-4;
            ...
            elseif matches( solverName, 'ipopt' )
                ...
                oc.ipopt.options.ipopt.tol = 1e-3;
                oc.ipopt.options.ipopt.constr_viol_tol = 1e-3;
                oc.ipopt.options.ipopt.dual_inf_tol = 1;
                oc.ipopt.options.ipopt.compl_inf_tol = 1e-3;

                bound_push_frac = 1e-8;
