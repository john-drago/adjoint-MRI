...
% (freely optimizable waveforms) with Fourier basis for RF
% waveforms and Chebyshev basis for gradient waveforms.
...
savePathTop = fullfile( tld, 'data', 'opt', ...
    '2025-02-03-fourier-cheb-test', strcat( 'opt', timestr ));
...
constraints = {...
    "total-RF-power", 500;... % W
    "max-RF-power", 500;... % W
    "RF-slew-rate", 5.5e7;... % V/s
    "RF-accel", 5e12;... % V/(sec^2)
    "RF-max", 600;... % V
    "peak-local-SAR", 1e3;... % W/kg
    "peak-global-SAR", 1e3;... % W/kg
    "average-local-SAR", 8;... % W/kg
    "average-global-SAR", 5;... % W/kg
    "grad-max", 15e-3;... % T/m
    "grad-slew-rate", 175;... % T/m/sec
...
% cheb specific pulse parameters
pulse.name = "fourier"; % e.g., mpptx, instant, pseudo-spectral-shim, pseudo-spectral,...
pulse.type = "cheb"; % e.g., base or extended for kTP

pulse.orderFourier_RF = 8;
pulse.orderCheb_grad = 6;
pulse.orderCheb_shim = 4;
pulse.length = 1e-3;
...
oc.gaopt.MaxGenerations = 4;
oc.gaopt.MaxStallGenerations = 10;
oc.gaopt.PopulationSize = 100;
...
oc.gaopt.InitialPopulationRange = 0.0001 * [ -1; 1 ];
