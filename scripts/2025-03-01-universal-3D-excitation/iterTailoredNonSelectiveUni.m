...
savePathTop = fullfile( mpd, 'data', 'opt', ...
    ...
    '2025-04-02-tailored-SPINS-init-10-SPINS',...
    strcat( 'opt', timestr ) );

prevOptDirPath = fullfile( mpd, "data", "opt", ...
    '2025-04-01-universal-nonselective',...
    "2025-04-01-universal-SPINS-init-10-SPINS",...
    "XXXXXXXXXXXXXXXX" );
...
for dd = 1:length( folderPaths )
    ...
    % if multiple dirs to iterate over
    folderPathDir = dir( folderPaths( dd ) );
    folderPathDirNames = transpose( string( extractfield( folderPathDir, "name" ) ) );

    if any( contains( folderPathDirNames, "180deg", 'IgnoreCase', true ) )
        loadPath = fullfile( folderPaths( dd ), "180deg" );
    else
        loadPath = fullfile( folderPaths( dd ) );
    end

    prev = load( fullfile( loadPath, 'optOutput.mat' ) );
