...
note = [...
    ...
    "Piecewise polynomial 90 degree flip angle" ];
...
savePathTop = fullfile( tld, 'data', 'opt', ...
    ...
    '2025-04-03-universal-spokes-init-90-pp',...
    strcat( 'opt', timestr ) );
...
for ll = 1:st.numSpokeLocations
    ...
    for ss = 1:length( st.solverNames )
        ...
        if st.initialGuessOnly
            ...
            maxOptTime = 30 * ( 60 * 60 );
            maxOptIterationsSQP = 6500;
            maxOptIterationsIP = 5000;
            ...
            if matches( solverName, 'active-set' )
                ...
                oc.fminopt.RelLineSrchBnd = 3.50e-4;
