...
note = [...
    ...
    "Piecewise polynomial 10 degree flip angle" ];
...
savePathTop = fullfile( tld, 'data', 'opt', ...
    ...
    '2025-04-03-universal-spokes-init-10-pp',...
    strcat( 'opt', timestr ) );
...
for ll = 1:st.numSpokeLocations
    ...
    ppcoeffs = getPPOptCoeffs( wvSpokes, optInit );

    p0 = zeros( optInit.numVars, 1 );
    p0( optInit.breal_idx ) = reshape( ppcoeffs.breal_coeffs, [ optInit.numVarsPerChannel_RF * optInit.numXYCoils, 1 ] );
    p0( optInit.bimag_idx ) = reshape( ppcoeffs.bimag_coeffs, [ optInit.numVarsPerChannel_RF * optInit.numXYCoils, 1 ] );
    p0( optInit.grad_idx ) = reshape( ppcoeffs.grad_coeffs, [ optInit.numVarsPerChannel_grad * 3, 1 ] );

    if optInit.numZCoils > 0
        p0( optInit.shim_idx ) = reshape( ppcoeffs.shim_coeffs, [ optInit.numVarsPerChannel_shim * optInit.numZCoils, 1 ] );
    end
    ...
    for ss = 1:length( st.solverNames )
        ...
        if st.initialGuessOnly
            ...
            maxOptTime = 30 * ( 60 * 60 );
            maxOptIterationsSQP = 2500;
            maxOptIterationsIP = 1500;
            ...
            if matches( solverName, 'active-set' )
                ...
                oc.fminopt.FunctionTolerance = 1e-10;
            ...
            elseif matches( solverName, 'ipopt' )
                ...
                oc.ipopt.options.ipopt.least_square_init_duals = 'yes';
