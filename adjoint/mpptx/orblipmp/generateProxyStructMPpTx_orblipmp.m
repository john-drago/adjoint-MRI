function popt = generateProxyStructMPpTx_orblipmp( opt, oc )
...
varArray = {...
    ...
    "Gyimag-MPSP", ( 1 );...
    "Gzimag-MPSP", ( 1 );...
    "shim-Blip", popt.numZCoils;...
    "shimreal-MPSP", popt.numZCoils;...
    "shimimag-MPSP", popt.numZCoils;...
    };
...
if isfield( oc, 'popt' )
    if isfield( oc.popt, 'zCoilScale' )
        if opt.numZCoils > 0
            zCoilIdxs = [ popt.shimreal_MPSP_idx; popt.shimimag_MPSP_idx; popt.shim_Blip_idx ];
...
if opt.numZCoils > 0
    vec( popt.shim_Blip_idx ) = opt.(vecName)( opt.shim_Blip_idx );
    vec( popt.shimreal_MPSP_idx ) = opt.(vecName)( opt.shimreal_MPSP_idx );
    vec( popt.shimimag_MPSP_idx ) = opt.(vecName)( opt.shimimag_MPSP_idx );
end
