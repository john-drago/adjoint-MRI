function [ waveform ] = generateMPpTxPlotWaveform_orblipmpconst_fixedphase( p, opt )
...
Freq = zeros( numXYCoils, ( numTimePoints ) );
...
bcomp_ORSP = complex( breal_ORSP, bimag_ORSP );
breal_MPSP = p( breal_MPSP_p_idx );
bimag_MPSP = p( bimag_MPSP_p_idx );
bcomp_MPSP = complex( breal_MPSP, bimag_MPSP );
...
% determine kTP indices interp
ORSP_Slew_rise_idx_interp = ( tvecInterp >= ( opt.tStORSP - dtError ) ) & ( tvecInterp <= ( opt.tStORSP + opt.RFSlewTime + dtError ) );
ORSP_Slew_fall_idx_interp = ( tvecInterp >= ( opt.tEndORSP - opt.RFSlewTime - dtError ) ) & ( tvecInterp <= ( opt.tEndORSP + dtError ) );
ORSP_const_idx_interp = ( tvecInterp >= ( opt.tStORSP + opt.RFSlewTime - dtError ) ) & ( tvecInterp <= ( opt.tEndORSP - opt.RFSlewTime + dtError ) );
ORSP_idx_interp = ( tvecInterp >= ( opt.tStORSP - dtError ) & ( tvecInterp <= ( opt.tEndORSP + dtError )  ) );

% rise
RFphasor( :, ORSP_Slew_rise_idx_interp ) = ( interp1(...
    round([ opt.tStORSP, tvecOrig( ORSP_Slew_i_idx ), (opt.tStORSP + opt.RFSlewTime) ], sigDigitTime, 'significant' ),...
    [ zeros( opt.numXYCoils, 1 ), ( bcomp_ORSP / opt.RFSlewTime ) * ( tvecOrig( ORSP_Slew_i_idx ) - opt.tStORSP ), bcomp_ORSP ].',...
    tvecInterp( ORSP_Slew_rise_idx_interp ) ) ).';
% fall
RFphasor( :, ORSP_Slew_fall_idx_interp ) = ( interp1(...
    round([ ( opt.tEndORSP - opt.RFSlewTime ), tvecOrig( ORSP_Slew_f_idx ), opt.tEndORSP ], sigDigitTime, 'significant' ),...
    [ bcomp_ORSP, - ( bcomp_ORSP / opt.RFSlewTime ) * ( tvecOrig( ORSP_Slew_f_idx ) - opt.tEndORSP ), zeros( opt.numXYCoils, 1 ) ].',...
    tvecInterp( ORSP_Slew_fall_idx_interp ) ) ).';
% constant
RFphasor( :, ORSP_const_idx_interp ) = ( interp1(...
    round([ ( opt.tStORSP + opt.RFSlewTime ), tvecOrig( ORSP_Const_idx ), ( opt.tEndORSP - opt.RFSlewTime ) ], sigDigitTime, 'significant' ),...
    repmat( bcomp_ORSP, [ 1, length( tvecOrig( ORSP_Const_idx ) ) + 2 ] ).',...
    tvecInterp( ORSP_const_idx_interp ) ) ).';

RF( :, ORSP_idx_interp ) = RFphasor( :, ORSP_idx_interp );
...
% determine kTP indices interp
MPSP_Slew_rise_idx_interp = ( tvecInterp >= ( opt.tStMPSP - dtError ) ) & ( tvecInterp <= ( opt.tStMPSP + opt.RFSlewTime + dtError ) );
MPSP_Slew_fall_idx_interp = ( tvecInterp >= ( opt.tEndMPSP - opt.RFSlewTime - dtError ) ) & ( tvecInterp <= ( opt.tEndMPSP + dtError ) );
MPSP_const_idx_interp = ( tvecInterp >= ( opt.tStMPSP + opt.RFSlewTime - dtError ) ) & ( tvecInterp <= ( opt.tEndMPSP - opt.RFSlewTime + dtError ) );
MPSP_idx_interp = ( tvecInterp >= ( opt.tStMPSP - dtError ) & ( tvecInterp <= ( opt.tEndMPSP + dtError )  ) );

% rise
RFphasor( :, MPSP_Slew_rise_idx_interp ) = ( interp1(...
    round([ opt.tStMPSP, tvecOrig( MPSP_Slew_i_idx ), (opt.tStMPSP + opt.RFSlewTime) ], sigDigitTime, 'significant' ),...
    [ zeros( opt.numXYCoils, 1 ), ( bcomp_MPSP / opt.RFSlewTime ) * ( tvecOrig( MPSP_Slew_i_idx ) - opt.tStMPSP ), bcomp_MPSP ].',...
    tvecInterp( MPSP_Slew_rise_idx_interp ) ) ).';
% fall
RFphasor( :, MPSP_Slew_fall_idx_interp ) = ( interp1(...
    round([ ( opt.tEndMPSP - opt.RFSlewTime ), tvecOrig( MPSP_Slew_f_idx ), opt.tEndMPSP ], sigDigitTime, 'significant' ),...
    [ bcomp_MPSP, - ( bcomp_MPSP / opt.RFSlewTime ) * ( tvecOrig( MPSP_Slew_f_idx ) - opt.tEndMPSP ), zeros( opt.numXYCoils, 1 ) ].',...
    tvecInterp( MPSP_Slew_fall_idx_interp ) ) ).';
% constant
RFphasor( :, MPSP_const_idx_interp ) = ( interp1(...
    round([ ( opt.tStMPSP + opt.RFSlewTime ), tvecOrig( MPSP_Const_idx ), ( opt.tEndMPSP - opt.RFSlewTime ) ], sigDigitTime, 'significant' ),...
    repmat( bcomp_MPSP, [ 1, length( tvecOrig( MPSP_Const_idx ) ) + 2 ] ).',...
    tvecInterp( MPSP_const_idx_interp ) ) ).';

RF( :, MPSP_idx_interp ) = RFphasor( :, MPSP_idx_interp ) .* exp( 1j * dwxy_mpptx * ( tvecInterp( MPSP_idx_interp ) - opt.tStMPSP ) );
...
% slew waveforms
Grad( :, Blip_Grad_slew_i_idx_interp ) = ( interp1(...
...
Grad( :, Blip_Grad_slew_f_idx_interp ) = ( interp1(...
...
Grad( :, Blip_Grad_const_idx_interp ) = ( interp1(...
...
if numZCoils > 0
    ...
    Shim_Blip = Shim_Blip_scale * Shim_mag_MPSP;
