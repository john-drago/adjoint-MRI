function [ oc, pulse, opt ] = processMPpTxPulse_orblipmp( oc, pulse, opt )
...
if strcmpi( opt.structtype, "opt" )
    varArray = {...
        ...
        "Gyimag-MPSP", ( 1 );...
        "Gzimag-MPSP", ( 1 );...
        "shim-Blip", opt.numZCoils;...
        "shimreal-MPSP", opt.numZCoils;...
        "shimimag-MPSP", opt.numZCoils;...
        };
    ...
    opt.estMaxActiveVarsTimeStep = 2 * opt.numXYCoils + 2 * 3 + 2 * opt.numZCoils;
    ...
    switch constraintList( cc )
        ...
        case { 'shim-blip-max' }
            ...
            shimMaxBlipidx = find( contains( varNames,...
                "shim-Blip",...
                'ignorecase', true ) );
            for ii = 1:length( shimMaxBlipidx )
                varIdx = varIdxs{ shimMaxBlipidx( ii ) };
                lb( varIdx ) = -1;
                ub( varIdx ) = 1;
                scVec( varIdx ) = shimBlipMax;
            end
        case { 'shim-slew-rate' }
...
for cc = 1:length( convConstraintList )
    switch convConstraintList( cc )
        ...
        case { 'shim-total' }
            ...
            [ nlconIneqSt ] = nlconAddList( nlconIneqSt, @constraintMPpTxPulseShimTotal, 1 );
...
opt.generatePlotWaveforms = @generateMPpTxPlotWaveform_orblipmp;
opt.generateWaveforms = @generateMPpTxWaveform_orblipmp;

%% Add function to calculate gradp_f array
opt.forwardModelGradientFunction = @forwardModelGradientMPpTx_orblipmp;

if opt.useGPU
    opt.gpuArrayAdjointFunction = @gpuArrayAdjointMPpTx_orblipmp;
...
%% Add proxy function
opt.generateProxy = @generateProxyStructMPpTx_orblipmp;
opt.generateProxyWaveform = @generateProxyMPpTxWaveform_orblipmp;
opt.generateFullFromProxy = @generateFullVecFromProxyMPpTx_orblipmp;
