...
note = [...
    ...
    "Piecewise constant 90 degree flip angle WITH bandwidth constraint" ];
...
savePathTop = fullfile( tld, 'data', 'opt',...
    ...
    '2025-04-04-tailored-spokes-init-90-pwc-con',...
    strcat( 'opt', timestr ) );

prevOptDirPath = fullfile( tld, "data", "opt",...
    "2025-04-03-universal-selective",...
    "2025-04-03-universal-spokes-init-90-pwc-con",...
    "XXXXXXXX" );
    ...
    for dd = 1:length( optTypeFolderPaths )
        ...
        for tt = 1:numTest
            ...
            elseif matches( solverName, 'ipopt' )
                ...
                if abs( sliceLocation - ( +0.0 * 1e-2 ) ) < 1e-12
