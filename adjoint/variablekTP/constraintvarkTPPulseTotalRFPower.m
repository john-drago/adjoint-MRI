function [ c, gradc ] = constraintvarkTPPulseTotalRFPower( pSc, opt )
...
if nargout > 1
    ...
    gradc_breal_sc_rshp = ( ( dutyCycle ) / ( 2 * opt.Z0 ) ) * gradc_breal_unsc .* breal_sc_rshp;
    ...
    gradc_bimag_sc_rshp = ( ( dutyCycle ) / ( 2 * opt.Z0 ) ) * gradc_bimag_unsc .* bimag_sc_rshp;
    ...
    gradc_dt_unsc = ( ( ( dutyCycle ) / ( 2 * opt.Z0 ) ) * ( sum( breal_rshp_pwr + bimag_rshp_pwr, 1) )...
        .* dt_RF_sc.' ) -...
        totalRFPower_constr * dt_RF_sc.';
    gradc_unsc( dt_RF_idx, 1 ) = gradc_dt_unsc;

    gradc_unsc( dt_blip_idx, 1) = - totalRFPower_constr * dt_blip_sc;
