function [ waveform ] = generatekTPPlotWaveform_base( p, opt )
...
for kk = 1:(num_kTP-1)
    ...
    Grad( :, Grad_idx_interp ) = grad_ktp_rshp( :, kk ) * ( 1 - abs(...
        ( tvecInterp( Grad_idx_interp ) - blipCenters( kk ) ) / ( blipLength / 2 ) ) );
...
if opt.numZCoils > 0
    ...
    for kk = 1:(num_kTP-1)
        ...
        Shim( :, Shim_idx_interp ) = shim_ktp_rshp( :, kk ) * ( 1 - abs(...
            ( tvecInterp( Shim_idx_interp ) - blipCenters( kk ) ) / ( blipLength / 2 ) ) );
