function [ c, gradc ] = constraintPPRFBandwidth( pSc, opt )
...
adjDFTmtxVarsByTP = opt.adjDFTmtxVarsByTP;
% numAdjDFTmtx = size( adjDFTmtxVarsByTP, 1 );
...
bcomp_rshp_fft = adjDFTmtxVarsByTP * bcomp_rshp;
...
if nargout > 1
    ...
    gradf_zbar_rshp = ...
        opt.adjdf * ctranspose( adjDFTmtxVarsByTP ) * ( bcomp_rshp_fft ./ bcomp_rshp_fft_abs ); 
    
    lin_breal_idx = sub2ind( [ opt.numVars, numXYCoils ],...
         breal_idx_rshp, repmat( 1:numXYCoils, [ numVarsPerChannel_RF, 1 ] ) );
    lin_bimag_idx = sub2ind( [ opt.numVars, numXYCoils ],...
         bimag_idx_rshp, repmat( 1:numXYCoils, [ numVarsPerChannel_RF, 1 ] ) );
