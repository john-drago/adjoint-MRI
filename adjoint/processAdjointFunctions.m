...
if isfield( pulse, 'excitationType' )
    if matches( pulse.excitationType, [ "slice-selective"; "sliceselective" ], 'IgnoreCase', true )
    ...
    elseif matches( pulse.excitationType, [ "non-selective"; "nonselective" ], 'IgnoreCase', true  )
...
if ~isempty( pulse.runningCostFunction )
    if matches( pulse.runningCostFunction, 'magnitude-least-squares', 'IgnoreCase', true  )
        error( "Don't have this function yet." )
        opt.runningCostFunction = @runMagnitudeLeastSquaresIntCost;
    else
