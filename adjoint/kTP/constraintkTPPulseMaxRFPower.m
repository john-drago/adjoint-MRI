function [ c, gradc ] = constraintkTPPulseMaxRFPower( pSc, opt )
...
maxpwr = dutyCycle/( 2 * opt.Z0 * pulseLength ) * ( highPower );

c_unsc = maxpwr - maxRFPower_constr;
...
if nargout > 1
    ...
    gradc_breal_sc = ( ( dutyCycle ) / ( 2 * opt.Z0 * pulseLength ) ) * gradc_breal_unsc .* breal_sc_rshp( highPowerChannel, : );
    ...
    gradc_bimag_sc = ( ( dutyCycle ) / ( 2 * opt.Z0 * pulseLength ) ) * gradc_bimag_unsc .* bimag_sc_rshp( highPowerChannel, : );
