...
note = [...
    ...
    "Chebyshev 10 degree flip angle" ];
...
savePathTop = fullfile( tld, 'data', 'opt', ...
    ...
    '2025-04-02-tailored-kTP-init-10-cheb',...
    strcat( 'opt', timestr ) );

prevOptDirPath = fullfile( tld, "data", "opt", ...
    '2025-04-01-universal-nonselective',...
    "2025-04-01-universal-kTP-init-10-cheb",...
    "XXXXXXXX" );
...
for dd = 1:length( folderPaths )
    ...
    for tt = 1:numTest
        ...
        elseif matches( solverName, 'ipopt' )
            ...
            oc.ipopt.options.ipopt.tol = 1e-8;
            ...
            oc.ipopt.options.ipopt.alpha_min_frac = 0.05;
            oc.ipopt.options.ipopt.max_soc = 4;
